...
import com.tim.WoodshopMC.Database.FMDB.FMDatabase;
...
// product
    ...
    public boolean isExistSameProduct(String productName) {
        String sql = "";
        sql = String.format("SELECT COUNT(*) AS samecount FROM tbl_product WHERE deleted = 0 and product_name = '%s'", productName);
        Cursor results = _database.executeCommand(sql);
        if (results.moveToFirst())
        {
            do {
    ...
    public void updateProductToDatabase(FSProduct product) {
        String sql;
        sql = String.format("UPDATE tbl_product SET product_name = '%s' WHERE product_id = %d", product.productName, product.productID);
        _database.executeUpdate(sql);
    }
    ...
    public FSProduct getProductWithLocProduct(FSLocProduct locProduct) {
        FSProduct product = new FSProduct();
        Cursor results = _database.executeQuery("SELECT * FROM tbl_product WHERE deleted = 0 AND product_name = '%s'", locProduct.locProductName);
        if (results.moveToFirst())
        {
            do {
                int i = 0;
...
// products for specific location
    ...
    public boolean isExistSameLocProduct(long locID, String locProductName) {
        String sql = "";
        sql = String.format("SELECT COUNT(*) AS samecount FROM tbl_locproduct WHERE deleted = 0 and locproduct_locid = %d and locproduct_productname = '%s'", locID, locProductName);
        Cursor results = _database.executeCommand(sql);
        if (results.moveToFirst())
        {
            do {
    ...
    public FSLocProduct getLocProductWithProduct(FSProduct product, long locID) {
        Cursor results = _database.executeQuery("SELECT * FROM tbl_locproduct WHERE deleted = 0 AND locproduct_locid = %d AND locproduct_productname = '%s'", locID, product.productName);
        if (results.moveToFirst())
        {
            do {
                int i = 0;
    ...
    public boolean updateLocProductToDatabase(FSLocProduct locProduct) {
        String sql;
        sql = String.format("UPDATE tbl_locproduct SET locproduct_locid = %d, locproduct_productname = '%s', locproduct_coverage = %f WHERE locproduct_id = %d", locProduct.locProductLocID, locProduct.locProductName, locProduct.locProductCoverage, locProduct.locProductID);
